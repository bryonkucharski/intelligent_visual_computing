...
def trainNN(X, Y, threshold=0.95, verbose = True, batch_size = 32):
    """
    ...
    for layer_id in range(1, model.num_layers):
        model.param[layer_id] = .1 * np.random.randn( model.num_nodes[layer_id-1],  model.num_nodes[layer_id]) # plus one unit (+1) for bias
    ...
    while 1:
        ...
        for layer_id in reversed(range(1,model.num_layers)):
            cost_function += (model.weight_decay* np.sum(model.param[layer_id] * model.param[layer_id])) #add l2 penalty to cost
            ...
            dA_prev, model.prev_grad[layer_id], model.db[layer_id] = backPropagate(message, model.param[layer_id], model.outputs[layer_id],model.outputs[layer_id-1], model.b[layer_id],model.weight_decay, model.activation, is_output_layer)
            model.prev_grad[layer_id] = model.prev_grad[layer_id] + (model.weight_decay  * model.param[layer_id]*2) #Add l2 reg
        ...
        for layer_id in range(1,model.num_layers):
            if model.update_method == 'GD':
                ...
                model.velocity[layer_id] = model.momentum * model.velocity[layer_id] - model.learning_rate * model.prev_grad[layer_id]  # integrate velocity
        ...
        if model.verbose and iter%50==0:
            print('Iteration %d, Cost function: %f, classification error: %f %%'%(iter, cost_function, classification_error * 100))
